...
export async function GET(
  ...
  try {
    ...
    const { data: attachment, error } = await supabaseServer
      ...
      .select('id, address_id, file_url, file_name, file_type')
      .eq('id', attachmentId)
      .eq('user_id', user.id)
      .single();
    ...
    const address = await loadOwnedAddress(attachment.address_id, user.id);
    if (!address) return NextResponse.json({ error: 'Address not found' }, { status: 404 });

    const storagePath = extractAddressBookStoragePath(attachment.file_url);
    if (!storagePath) {
      return NextResponse.json({ error: 'Invalid file path' }, { status: 500 });
    }

    const { data: fileData, error: downloadError } = await supabaseServer.storage
      .from(ADDRESS_BOOK_BUCKET)
