...
import {
  ...
  loadOwnedCompletion,
  loadOwnedItem,
  mapCleaningAttachment,
  removeCleaningStorageFiles,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error: uploadError } = await supabaseServer.storage.from(CLEANING_SCHEDULE_BUCKET).upload(storagePath, buffer, {
    ...
    const { data: urlData } = supabaseServer.storage.from(CLEANING_SCHEDULE_BUCKET).getPublicUrl(storagePath);
    const table = ownerKind === 'item' ? 'tools_cs_item_attachments' : 'tools_cs_completion_attachments';
    ...
    if (insertError || !attachment) {
      await supabaseServer.storage.from(CLEANING_SCHEDULE_BUCKET).remove([storagePath]);
    ...
    return NextResponse.json({ attachment: mapCleaningAttachment(attachment) });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (itemAttachment) {
      ...
      await removeCleaningStorageFiles([itemAttachment.file_url], user.id);
