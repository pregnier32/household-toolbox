...
export async function GET(
  ...
  try {
    ...
    const { data: attachment, error } = await supabaseServer
      ...
      .select('id, meal_id, file_url, file_name, file_type')
      .eq('id', attachmentId)
      .eq('user_id', user.id)
      .single();
    ...
    const meal = await loadOwnedMeal(attachment.meal_id, user.id);
    if (!meal) return NextResponse.json({ error: 'Meal not found' }, { status: 404 });

    const storagePath = extractMealPlannerStoragePath(attachment.file_url);
    if (!storagePath) {
      return NextResponse.json({ error: 'Invalid file path' }, { status: 500 });
    }

    const { data: fileData, error: downloadError } = await supabaseServer.storage
      .from(MEAL_PLANNER_BUCKET)
