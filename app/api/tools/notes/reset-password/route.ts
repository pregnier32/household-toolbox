...
// GET - Fetch security questions for a note (without answers)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: note, error: noteError } = await supabaseServer
      ...
      .select('id, requires_password_for_view')
      .eq('id', noteId)
      .eq('user_id', user.id)
      .single();
    ...
    console.error('Error in GET /api/tools/notes/reset-password:', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error: updateError } = await supabaseServer
      .from('tools_note_notes')
      .update({ view_password_hash: newPasswordHash })
      .eq('id', noteId)
      .eq('user_id', user.id);
    ...
    console.error('Error in POST /api/tools/notes/reset-password:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
