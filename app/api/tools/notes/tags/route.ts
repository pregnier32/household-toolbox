...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('Error in GET /api/tools/notes/tags:', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('Error in POST /api/tools/notes/tags:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const { data: noteTags, error: checkError } = await supabaseServer
    ...
    if (noteTags && noteTags.length > 0) {
      return NextResponse.json(
        { error: 'Cannot delete tag that is in use by notes' },
        { status: 400 }
      );
    }
    ...
    console.error('Error in DELETE /api/tools/notes/tags:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
