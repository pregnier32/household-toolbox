...
import {
  extractShoppingListStoragePath,
...
export async function GET(
  ...
  try {
    ...
    const { data: attachment, error } = await supabaseServer
      ...
      .select('id, list_id, file_url, file_name, file_type')
      .eq('id', attachmentId)
      .eq('user_id', user.id)
      .single();
    ...
    const list = await loadOwnedList(attachment.list_id, user.id);
    if (!list) return NextResponse.json({ error: 'List not found' }, { status: 404 });

    const storagePath = extractShoppingListStoragePath(attachment.file_url);
    if (!storagePath) {
      return NextResponse.json({ error: 'Invalid file path' }, { status: 500 });
    }

    const { data: fileData, error: downloadError } = await supabaseServer.storage
      .from(SHOPPING_LIST_BUCKET)
