...
import {
  extractSubscriptionTrackerStoragePath,
  isMissingRelationError,
  loadOwnedSubscription,
  SUBSCRIPTION_TRACKER_BUCKET,
...
export async function GET(
  ...
  try {
    ...
    const { data: attachment, error } = await supabaseServer
      ...
      .select('id, subscription_id, file_url, file_name, file_type')
      .eq('id', attachmentId)
      .eq('user_id', user.id)
      .single();
    ...
    const subscription = await loadOwnedSubscription(attachment.subscription_id, user.id);
    if (!subscription) return NextResponse.json({ error: 'Subscription not found' }, { status: 404 });

    const storagePath = extractSubscriptionTrackerStoragePath(attachment.file_url);
    if (!storagePath) {
      return NextResponse.json({ error: 'Invalid file path' }, { status: 500 });
    }

    const { data: fileData, error: downloadError } = await supabaseServer.storage
      .from(SUBSCRIPTION_TRACKER_BUCKET)
