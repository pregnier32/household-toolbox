...
import {
  ...
  mapSubscriptionAttachment,
  removeSubscriptionTrackerStorageFiles,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!toolId || !subscriptionId) {
      return NextResponse.json({ error: 'Tool ID and Subscription ID are required' }, { status: 400 });
    }
    if (!file || file.size <= 0) {
    ...
    const subscription = await loadOwnedSubscription(subscriptionId, user.id, toolId);
    if (!subscription) return NextResponse.json({ error: 'Subscription not found' }, { status: 404 });
    if (subscription.is_active === false) {
      return NextResponse.json({ error: 'History subscriptions are view-only. Reactivate to add files.' }, { status: 403 });
    }

    await assertCanStoreBytes(user.id, file.size);

    const sanitized = file.name.replace(/[^a-zA-Z0-9.-]/g, '_');
    const storagePath = `${user.id}/${subscriptionId}/${Date.now()}-${sanitized}`;
    const buffer = Buffer.from(await file.arrayBuffer());
    const { error: uploadError } = await supabaseServer.storage.from(SUBSCRIPTION_TRACKER_BUCKET).upload(storagePath, buffer, {
    ...
    const { data: urlData } = supabaseServer.storage.from(SUBSCRIPTION_TRACKER_BUCKET).getPublicUrl(storagePath);
    ...
    if (insertError || !attachment) {
      await supabaseServer.storage.from(SUBSCRIPTION_TRACKER_BUCKET).remove([storagePath]);
    ...
    return NextResponse.json({ attachment: mapSubscriptionAttachment(attachment) });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const { data: attachment, error: attachmentError } = await supabaseServer
      .from('tools_st_subscription_attachments')
      .select('id, subscription_id, file_url')
      .eq('id', attachmentId)
      .eq('user_id', user.id)
      .single();
    ...
    const subscription = await loadOwnedSubscription(attachment.subscription_id, user.id, toolId);
    if (!subscription) return NextResponse.json({ error: 'Subscription not found' }, { status: 404 });
    if (subscription.is_active === false) {
      return NextResponse.json({ error: 'History subscriptions are view-only. Reactivate to remove files.' }, { status: 403 });
    }
    ...
    await removeSubscriptionTrackerStorageFiles([attachment.file_url], user.id);
