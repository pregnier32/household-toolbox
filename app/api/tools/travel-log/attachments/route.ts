...
import {
  ...
  loadOwnedTrip,
  mapTravelLogAttachment,
  removeTravelLogStorageFiles,
  TRAVEL_LOG_BUCKET,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tripId = (formData.get('tripId') as string | null)?.trim() || '';
    const file = formData.get('file') as File | null;

    if (!toolId || !tripId) {
      return NextResponse.json({ error: 'Tool ID and Trip ID are required' }, { status: 400 });
    }
    if (!file || file.size <= 0) {
    ...
    const trip = await loadOwnedTrip(tripId, user.id, toolId);
    if (!trip) return NextResponse.json({ error: 'Trip not found' }, { status: 404 });
    await assertCanStoreBytes(user.id, file.size);

    const sanitized = file.name.replace(/[^a-zA-Z0-9.-]/g, '_');
    const storagePath = `${user.id}/${tripId}/${Date.now()}-${sanitized}`;
    const buffer = Buffer.from(await file.arrayBuffer());
    const { error: uploadError } = await supabaseServer.storage.from(TRAVEL_LOG_BUCKET).upload(storagePath, buffer, {
    ...
    const { data: urlData } = supabaseServer.storage.from(TRAVEL_LOG_BUCKET).getPublicUrl(storagePath);
    ...
    if (insertError || !attachment) {
      await supabaseServer.storage.from(TRAVEL_LOG_BUCKET).remove([storagePath]);
    ...
    return NextResponse.json({ attachment: mapTravelLogAttachment(attachment) });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const trip = await loadOwnedTrip(attachment.trip_id, user.id, toolId);
    if (!trip) return NextResponse.json({ error: 'Trip not found' }, { status: 404 });
    ...
    await removeTravelLogStorageFiles([attachment.file_url], user.id);
