...
import type { HmsAttachment } from '@/lib/home-maintenance-schedule';

export const HOME_MAINTENANCE_SCHEDULE_BUCKET = 'home-maintenance-schedule';

export function isMissingRelationError(error: { code?: string; message?: string } | null | undefined): boolean {
  return error?.code === '42P01' || (error?.message || '').toLowerCase().includes('does not exist');
}

export function extractHmsStoragePath(fileUrl: string | null | undefined): string | null {
  if (!fileUrl) return null;
  if (!fileUrl.startsWith('http://') && !fileUrl.startsWith('https://')) return fileUrl;
  try {
    const url = new URL(fileUrl);
    const marker = `${HOME_MAINTENANCE_SCHEDULE_BUCKET}/`;
...
export async function removeHmsStorageFiles(fileUrls: Array<string | null | undefined>, userId: string): Promise<void> {
  const paths = fileUrls.map(extractHmsStoragePath).filter((path): path is string => Boolean(path));
  if (paths.length === 0) return;
  await supabaseServer.storage.from(HOME_MAINTENANCE_SCHEDULE_BUCKET).remove(paths);
  await refreshUserStorageUsage(userId);
}

export function mapHmsAttachment(row: {
  id: string;
  file_name: string;
  file_size: number;
  file_type: string;
...
export async function attachmentsByItemIds(itemIds: string[], userId: string) {
  const map: Record<string, HmsAttachment[]> = {};
  ...
  if (error) {
    if (!isMissingRelationError(error)) console.error('Error fetching home maintenance item attachments:', error);
    return map;
  }

  (data || []).forEach((row) => {
    if (!map[row.item_id]) map[row.item_id] = [];
    map[row.item_id].push(mapHmsAttachment(row));
  });
  return map;
}
...
export async function attachmentsByCompletionIds(completionIds: string[], userId: string) {
  ...
  if (error) {
    if (!isMissingRelationError(error)) console.error('Error fetching home maintenance completion attachments:', error);
    return map;
  }

  (data || []).forEach((row) => {
    if (!map[row.completion_id]) map[row.completion_id] = [];
    map[row.completion_id].push(mapHmsAttachment(row));
  });
  return map;
}
...
export async function deleteItemStorageFiles(itemId: string, userId: string): Promise<void> {
  ...
  if (itemError && !isMissingRelationError(itemError)) {
    console.error('Error loading home maintenance item files for delete:', itemError);
  }
  ...
  if (taskError && !isMissingRelationError(taskError)) {
    console.error('Error loading home maintenance tasks for item file delete:', taskError);
  }
  ...
  if (taskIds.length > 0) {
    ...
    if (completionError && !isMissingRelationError(completionError)) {
      console.error('Error loading home maintenance completions for item file delete:', completionError);
    }
    ...
    if (completionIds.length > 0) {
      ...
      if (completionFileError && !isMissingRelationError(completionFileError)) {
        console.error('Error loading home maintenance completion files for item delete:', completionFileError);
      }
      urls.push(...(completionFiles || []).map((row) => row.file_url));
    }
  }

  await removeHmsStorageFiles(urls, userId);
}
...
export async function deleteTaskCompletionStorageFiles(taskId: string, userId: string): Promise<void> {
  ...
  if (completionError) {
    if (!isMissingRelationError(completionError)) {
      console.error('Error loading home maintenance completions for task file delete:', completionError);
    }
    return;
  }
  ...
  const { data, error } = await supabaseServer
    ...
    .in('completion_id', completionIds);
  if (error) {
    if (!isMissingRelationError(error)) console.error('Error loading home maintenance completion files for task delete:', error);
    return;
  }
  await removeHmsStorageFiles((data || []).map((row) => row.file_url), userId);
}
