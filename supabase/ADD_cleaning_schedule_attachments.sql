-- Cleaning Schedule attachments: standing files on library items,
-- plus dated proof on completion occurrences.
-- Safe to re-run. After this succeeds, reload types:
--   npm run supabase:types
--
-- Files live in the existing `cleaning-schedule` storage bucket at
...
-- so task-level files would duplicate the item store.

CREATE TABLE IF NOT EXISTS tools_cs_item_attachments (
...
CREATE INDEX IF NOT EXISTS idx_cs_item_attachments_item_id ON tools_cs_item_attachments(item_id);
CREATE INDEX IF NOT EXISTS idx_cs_item_attachments_user_id ON tools_cs_item_attachments(user_id);

ALTER TABLE tools_cs_item_attachments ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own cleaning item attachments" ON tools_cs_item_attachments;
...
CREATE TABLE IF NOT EXISTS tools_cs_completion_attachments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  completion_id UUID NOT NULL REFERENCES tools_cs_completions(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  file_url TEXT NOT NULL,
  file_name TEXT NOT NULL,
  file_size INTEGER NOT NULL CHECK (file_size >= 0),
...
CREATE INDEX IF NOT EXISTS idx_cs_completion_attachments_completion_id ON tools_cs_completion_attachments(completion_id);
CREATE INDEX IF NOT EXISTS idx_cs_completion_attachments_user_id ON tools_cs_completion_attachments(user_id);

ALTER TABLE tools_cs_completion_attachments ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own cleaning completion attachments" ON tools_cs_completion_attachments;
